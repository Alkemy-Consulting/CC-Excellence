...
if not st.session_state.data_loaded:
    ...
    # Ensure data cleaning is performed if data is loaded
    if st.session_state.data_loaded and st.session_state.cleaned_data is None:
        st.warning("⚠️ **Data cleaning not completed!** Please complete data preprocessing in the sidebar.")
        st.stop()
    
    
    # Execute forecast when run_forecast flag is set - SHOW RESULTS BELOW ANALYSIS
    ...
    # Show ready to forecast message if no results available yet
    if not st.session_state.get('forecast_results_available', False):
        st.markdown("---")
